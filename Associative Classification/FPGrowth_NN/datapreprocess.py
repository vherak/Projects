...
def preprocess_data(path_to_data, header=None, missing_value_symbol=None, numeric_columns=None, remove_columns=None):
    ...
    if numeric_columns is not None:
        ...
        # print("After standardization: ", pd.DataFrame(data))
        discretize_data(data, numeric_columns)
        # print("After discretization: ", pd.DataFrame(data))
...
def chi_square(data, class_column):
    ...
    features = data.drop(columns=class_column, axis=1)
    feature_columns = list(features.columns)
    chi_value, _ = chi2(features, target)
    chi_value = pd.DataFrame(chi_value)
    chi_value = chi_value.transpose()
    chi_value.columns = feature_columns
    return chi_value
...
if __name__ == "__main__":
    ...
    ig = information_gain(pd.DataFrame(breast_cancer.copy()), 0)
    corr = correlation(breast_cancer.copy(), 0)
    chi2 = chi_square(breast_cancer.copy(), 0)


    print("information_gain", ig)
