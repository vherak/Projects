...
from datapreprocess import correlation, chi_square
import pandas as pd
...
def get_classification_association_rules(training_data, class_label, support_threshold, confidence_threshold):
    ...
    Args:
        ...
        num_transactions (int, optional): It is used to calculate the support of the rule. Defaults to None.
...
def rankRule_IG(data, CARs, info_gain, class_column, feature_info, use_conf = False, use_supp = False, use_conf_supp = False):
    ranked_CARs = []
    corr = correlation(data, class_column)
    chi2 = chi_square(data, class_column)
    ...
    for rule in CARs:
        ...
        for i in range(len(key_list)):
            for item in info_gain:
                if key_list[i] == item:
                    ...
                    # for chi squared
                    total_chi += chi2[key_list[i]]

        average_ig = total_ig/len(key_list)
        average_corr = total_corr/len(key_list)
        average_chi = total_chi/len(key_list)
        # print(average_ig, average_corr,average_chi )

        accuracy = 0
        item = list(rule) + list(consequent)
        for row in data:
            if all(i in row for i in item):
                accuracy += 1
        average_acc = accuracy/len(data)
        ...
        # interestingess = math.log2(1/average_ig) + math.log2(1/average_chi)

        # if average_corr < 0:
        #     interestingess = - math.log2(1/-average_corr) + math.log2(1/average_chi)
        # else:
        #     interestingess = math.log2(1/average_corr) + math.log2(1/average_chi)

        # if average_corr < 0:
        #     interestingess = math.log2(1/average_ig) - math.log2(1/-average_corr) + math.log2(1/average_chi)
        # else:
        #     interestingess = math.log2(1/average_ig) + math.log2(1/average_corr) + math.log2(1/average_chi)


        # print(average_ig, average_acc, interestingess)
    ...
    # print( "These are the Top 50 rules that is ranked using IG, Confidence and Support ")
    # for _, antecedent, consequent, confidence , support, interestingness in ranked_CARs:
    #     ranked_CARs_dict[antecedent] = (consequent, confidence)
    #     if count <= 50 :
    #         print( "RULE", count, " : ", antecedent, " ---> ", consequent, "[ I :", interestingness , ", C :", confidence, ", S :", support, "]")
    #         count += 1
...
if __name__ == "__main__":
    ...
    # dataset imports
    # breast_cancer = preprocess_data("./Datasets/breast-cancer.data", missing_value_symbol='?')
    # primary_tumour = preprocess_data("./Datasets/primary-tumor.csv")
    # balance_scale = preprocess_data("./Datasets/balance-scale.data")
    # car = preprocess_data("./Datasets/car.data")
    ...
    # breast_cancer_rules = get_classification_association_rules(breast_cancer, breast_cancer_class_label, 10, 0.5)
    ...
    # balance_scale_rules = get_classification_association_rules(balance_scale, balance_scale_class_label, 20, 0.5)
    # print(len(balance_scale_rules))
    # car_rules = get_classification_association_rules(car, car_class_label, 30, 0.5)
    # print(len(car_rules))

    # for rule in breast_cancer_rules:
    #     consequent, confidence = breast_cancer_rules.get(rule)
    ...
    #     consequent, confidence = balance_scale_rules.get(rule)
    #     print(rule, "=>", consequent, "[", confidence, "]")

    # for rule in car_rules:
    #     consequent, confidence = car_rules.get(rule)
    #     print(rule, "=>", consequent, "[", confidence, "]")